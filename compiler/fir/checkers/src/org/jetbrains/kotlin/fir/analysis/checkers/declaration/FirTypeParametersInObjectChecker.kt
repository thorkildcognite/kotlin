...
object FirTypeParametersInObjectChecker : FirClassChecker() {
    override fun check(declaration: FirClass, context: CheckerContext, reporter: DiagnosticReporter) {
        ...
        if (declaration is FirRegularClass && declaration.typeParameters.isNotEmpty()) {
            reporter.reportOn(declaration.source, FirErrors.TYPE_PARAMETERS_IN_OBJECT, context)
        } else if (declaration is FirAnonymousObject && declaration.source?.getChild(KtNodeTypes.TYPE_PARAMETER_LIST, depth = 1) != null) {
